...
def load_flow_demo_1(n_train, n_test, loader_args, visualize=True, train_only=False):
    ...
    train_data, test_data = generate_1d_flow_data(n_train), generate_1d_flow_data(
        n_test
    )
    ...
    if visualize:
        ...
        densities = 0.5 * norm.pdf(x, loc=-1, scale=0.25) + 0.5 * norm.pdf(
            x, loc=0.5, scale=0.5
        )
        plt.figure()
    ...
    train_loader, test_loader = (
        data.DataLoader(train_dset, **loader_args),
        data.DataLoader(test_dset, **loader_args),
    )
...
def load_flow_demo_2(
    ...
    if distribution == "uniform":
        ...
        train_data = np.random.choice(
            np.linspace(-2, 2, 100), size=n_train, p=ys / ys.sum()
        )
        test_data = np.random.choice(
            np.linspace(-2, 2, 100), size=n_test, p=ys / ys.sum()
        )
...
def load_flow_demo_3(
    n_train, n_test, loader_args, visualize=True, train_only=False, distribution="face"
):
    ...
    for i in range(len(n_layers)):
        for j in range(len(n_components)):
            text = ax.text(
                j,
                i,
                "{:0.2f}".format(flipped_losses[i, j]),
                ha="center",
                va="center",
                color="w",
            )
