...
def q2_save_results(dset_type, part, fn):
    ...
    if part == "a":
        if dset_type == 1:
            ...
            n, d = 100000, 200
            true_dist, data = q2_a_sample_data(
                join(data_dir, "geoffrey-hinton.jpg"), n, d
            )
        else:
            raise Exception("Invalid dset_type:", dset_type)
        split = int(0.8 * len(data))
    ...
    if part == "a":
        train_losses, test_losses, distribution = fn(
            train_data, test_data, d, dset_type
        )
        assert np.allclose(
            np.sum(distribution), 1
        ), f"Distribution sums to {np.sum(distribution)} != 1"
...
def q3a_save_results(dset_type, q3_a):
    ...
    save_training_plot(
        train_losses,
        test_losses,
        f"Q3(a) Dataset {dset_type} Train Plot",
...
def q3d_save_results(dset_type, q3_d):
    ...
    train_losses, test_losses, samples = q3_d(
        train_data,
        train_labels,
        test_data,
        test_labels,
    ...
    save_training_plot(
        train_losses,
        test_losses,
        f"Q3(d) Dataset {dset_type} Train Plot",
...
def q4a_save_results(q4_a):
    ...
    print(f"Final Test Loss: {test_losses[-1]:.4f}")
    save_training_plot(
        train_losses, test_losses, f"Q4(a) Train Plot", f"results/q4_a_train_plot.png"
    )
...
def q4b_save_results(q4_b):
    ...
    train_data, test_data = load_pickled_data(join(data_dir, "mnist_colored.pkl"))
    img_shape = (28, 28, 3)
    train_losses, test_losses, gray_samples, color_samples = q4_b(
        train_data, test_data, img_shape
    )
    gray_samples, color_samples = gray_samples.astype("float32"), color_samples.astype(
        "float32"
    )
    gray_samples *= 255
    ...
    print(f"Final Test Loss: {test_losses[-1]:.4f}")
    save_training_plot(
        train_losses, test_losses, f"Q4(b) Train Plot", f"results/q4_b_train_plot.png"
    )
...
def q4c_save_results(q4_c):
    ...
    train_data, test_data = (
        train_data.permute(0, 2, 3, 1).numpy(),
        test_data.permute(0, 2, 3, 1).numpy(),
    )
    train_data, test_data = (train_data > 0.5).astype("uint8"), (
        test_data > 0.5
    ).astype("uint8")
    ...
    print(f"Final Test Loss: {test_losses[-1]:.4f}")
    save_training_plot(
        train_losses, test_losses, f"Q4(c) Train Plot", f"results/q4_c_train_plot.png"
    )
