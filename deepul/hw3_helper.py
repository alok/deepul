...
def q3_save_results(dset_id, fn):
    ...
    (
        vqvae_train_losses,
        vqvae_test_losses,
        pixelcnn_train_losses,
        pixelcnn_test_losses,
        samples,
        reconstructions,
    ...
    save_training_plot(
        pixelcnn_train_losses,
        pixelcnn_test_losses,
        f"Q3 Dataset {dset_id} PixelCNN Prior Train Plot",
...
def q4_a_save_results(dset_id, fn):
    ...
    save_training_plot(
        vqvae_train_losses,
        vqvae_test_losses,
        f"Q4(a) Dataset {dset_id} VQ-VAE Train Plot",
    ...
    show_samples(
        samples,
        title=f"Q4(a) Dataset {dset_id} Samples",
        fname=f"results/q4_a_dset{dset_id}_samples.png",
    )
    show_samples(
        reconstructions,
        title=f"Q4(a) Dataset {dset_id} Reconstructions",
...
def q4_b_save_results(fn):
    ...
    samples, reconstructions = (
        samples.astype("float32") * 255,
        reconstructions.astype("float32") * 255,
    )
    print(
        f"Final -ELBO: {test_losses[-1, 0]:.4f}, Recon Loss: {test_losses[-1, 1]:.4f}, "
        f"KL Loss: {test_losses[-1, 2]:.4f}"
    ...
    plot_vae_training_plot(
        train_losses,
        test_losses,
        f"Q4({part}) Train Plot",
        f"results/q4_{part}_train_plot.png",
    )
    show_samples(
        samples, title=f"Q4({part}) Samples", fname=f"results/q4_{part}_samples.png"
    )
    show_samples(
        reconstructions,
        title=f"Q4({part}) Reconstructions",
